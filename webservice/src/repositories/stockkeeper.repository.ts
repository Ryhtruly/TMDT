...
export class StockkeeperRepository {
  ...
  async findOrderByTracking(tracking_code: string) {
    const result = await pool.query(`
      SELECT o.*, 
             wi.id_hub as current_hub, wi.id_spoke as current_spoke, wi.shelf_location, wi.last_updated as last_warehouse_update
      FROM orders o
  ...
  async getOverdueAlerts(id_hub: number | null, id_spoke: number | null) {
    const result = await pool.query(`
      SELECT wi.*, o.tracking_code, o.receiver_name, o.receiver_phone,
